...
impl Hand {
    ...
    fn cmp(&self, other_hand: &Hand) -> Ordering {
        ...
        match (self.card_type.as_ref().unwrap() , other_hand.card_type.as_ref().unwrap()) {
            (Types::FiveKind(a), Types::FiveKind(b)) => a.cmp(&b),
            (Types::FiveKind(_a), _) => Ordering::Greater,
            (_, Types::FiveKind(_a)) => Ordering::Less,
            (Types::FourKind(_a), Types::FourKind(_b)) => compare_string(&self.desc, &other_hand.desc),
            (Types::FourKind(_a),_) => Ordering::Greater,
            (_, Types::FourKind(_a)) => Ordering::Less,
            (Types::FullHouse((_a1,_a2)), Types::FullHouse((_b1,_b2))) => compare_string(&self.desc, &other_hand.desc),
            (Types::FullHouse((_a1,_a2)), _ ) => Ordering::Greater,
            (_, Types::FullHouse((_a1,_a2)) ) => Ordering::Less,
            (Types::ThreeKind(_a), Types::ThreeKind(_b) ) => compare_string(&self.desc, &other_hand.desc),
            (Types::ThreeKind(_a), _ ) => Ordering::Greater,
            (_, Types::ThreeKind(_b) ) => Ordering::Less,
            (Types::TwoPairs((_a1,_a2)), Types::TwoPairs((_b1,_b2))) =>compare_string(&self.desc, &other_hand.desc),
            (Types::TwoPairs((_a1,_a2)), _) => Ordering::Greater,
            (_, Types::TwoPairs((_b1,_b2))) => Ordering::Less,
            (Types::OnePair(_a), Types::OnePair(_b)) => compare_string(&self.desc, &other_hand.desc),
    ...
    fn parse_type(&mut self) {
        ...
        let type_code = cards_array.iter().fold(String::from(""),|acc, (_, n)| format!("{}{}",acc,n) );

        match type_code.as_str() {
            "11111" => self.card_type = Some(Types::HighCard),
            "2111" => self.card_type = Some(Types::OnePair(*cards_array[0].0)),
            "221" => self.card_type = Some(Types::TwoPairs((*cards_array[0].0,*cards_array[1].0))),
            "311" => self.card_type = Some(Types::ThreeKind(*cards_array[0].0)),
            "32" => self.card_type = Some(Types::FullHouse((*cards_array[0].0,*cards_array[1].0))),
            "41" => self.card_type = Some(Types::FourKind(*cards_array[0].0)),
            "5" => self.card_type = Some(Types::FiveKind(*cards_array[0].0)),
